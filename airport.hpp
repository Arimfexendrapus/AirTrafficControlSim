...
public:
	airport(); // constructor
	bool degree_clearance(string runwayID, vector<int> coords, int aircraftHeading);
...
bool airport::degree_clearance(string runwayID, vector<int> coords, int aircraftHeading)
{
	...
	if (refAngle <= 30)
	{
		if (aircraftHeading > runways[i].heading - 30)
		{
			if (aircraftHeading < runways[i].heading + 30)
			{
				return true;
